...
module.exports = {
  getAppPositionByKeyword: function(bundleId, keyword, cb) {
    //var baseUrl = //'https://play.google.com/store/search?q=dieta';
    //https://play.google.com/store/search?q=dieta&c=apps
    var searchUrl = 'https://play.google.com/store/search?q='+keyword+'&c=apps';
    ...
    request(searchUrl, function(err, res, html) {
        ...
        var position = 0;
        $('div.card').each(function(i, item) {  
          //console.log(item);
          position = position+1;
          //console.log(item.attribs['data-docid']);

          if (item.attribs['data-docid'] == bundleId) {
            return false;
          }
          /*
